...
def get_datastream(offset, num_batches):
    ...
    for i in xrange(num_batches):
        ...
        for j in xrange(BATCH_SIZE):
            ...
            #for i in xrange(len(reshaped_padded_audio)):
            #    reshaped_padded_audio[i] = reshaped_padded_audio[i] / ((max(reshaped_padded_audio[i]) - min(reshaped_padded_audio[i])) / 2.0) + 1
            #    print max(reshaped_padded_audio[i]), min(reshaped_padded_audio[i])
...
savefname = "saved_params/rnn_ctc.pickle"
...
algorithm = blalg.GradientDescent(
    ...
    step_rule=blalg.Adam(learning_rate=0.002)
)
...
main_loop = blml.MainLoop(algorithm, stream_train,
    extensions=[
        ...
        blext.Printing(),
        blext.ProgressBar(),
        extensions.SaveBestModel("saved_params/rnn_ctc.pickle", [input_layer, recurrent_layer, output_layer])
    ]
)  
