...
def pseudo_cost(y, y_hat, y_mask, y_hat_mask):
    """
    Training objective.
...
def cost(y, y_hat_softmax, y_mask, y_hat_mask):
    """
    Training objective.
    ...
    """
    if y_mask is None:
        y_mask = T.ones(y.shape,dtype=theano.config.floatX)
    if y_hat_mask is None:
        y_hat_mask = T.ones((y_hat_softmax.shape[0], y_hat_softmax.shape[1]),
